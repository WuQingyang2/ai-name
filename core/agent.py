...
个性化定制：根据用户具体需求（如性别倾向、字数限制、风格偏好——儒雅/清丽/大气/灵动等），提供5个候选方案。
请只返回符合以下结构的 JSON，不要输出 Markdown 或额外解释：
{
  "names": [
    {
      "name": "姓名",
...
async def generate_names(name_info: NameIn) -> NameResultSchema:
    exclude_names = "、".join(name_info.exclude)
    content = f"用户姓氏是：{name_info.surname}，性别是：{name_info.gender}，名字字数要求是：{name_info.length}，其他要求为：{name_info.other}，以下名字不要选：{exclude_names}"
